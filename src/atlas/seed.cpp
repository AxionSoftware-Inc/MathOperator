...
Atlas make_vector_calculus_seed() {
  ...
  Identity i1{"identity.laplacian","Laplacian definition",Expression::ref("op.laplacian"),Expression::composition(Expression::ref("op.divergence"),Expression::ref("op.gradient")),{"Euclidean R3","metric"},{"3"},{"C2"},{}, {"seed"},VerificationStatus::Proposed,{}}; i1.executable_equality=true; i1.evidence.push_back({"e.laplacian.type","type_checked","seed","0.1","2026-08-15","seed","accepted","",-1}); i1.verification=derive_status(i1.evidence); a.add_identity(i1);
  Identity cg{"identity.curl.grad.zero","curl(grad(f)) = 0",Expression::composition(Expression::ref("op.curl.3d"),Expression::ref("op.gradient")),Expression::ref("op.zero.scalar_to_vector.r3"),{"Euclidean R3","metric","orientation"},{"3"},{"C2"},{}, {"seed"},VerificationStatus::Proposed,{}}; cg.executable_equality=true; a.add_identity(cg);
  Identity dc{"identity.div.curl.zero","div(curl(F)) = 0",Expression::composition(Expression::ref("op.divergence"),Expression::ref("op.curl.3d")),Expression::ref("op.zero.vector_to_scalar.r3"),{"Euclidean R3","metric","orientation"},{"3"},{"C2"},{}, {"seed"},VerificationStatus::Proposed,{}}; dc.executable_equality=true; a.add_identity(dc);
  return a;
}
}
